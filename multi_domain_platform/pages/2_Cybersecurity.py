...
with st.form("create_incident_form"):
    ...
    status = st.selectbox("Status", ["Open", "Investigating", "Resolved", "Closed", "Mitigated"])
    date = st.date_input("Date")

    submitted = st.form_submit_button("Create Incident")

    if submitted:
        db.create_security_incident(title, severity, status, date)
...
for incident in incidents:
    with st.expander(f"Incident #{incident.get_id()} — {incident.get_title()}"):
        ...
        new_severity = st.selectbox(
            ...
            key=f"sev_{incident.get_id()}"
        )

        new_status = st.selectbox(
            "Status",
            ["Open", "Investigating", "Resolved", "Closed", "Mitigated"],
            index=["Open", "Investigating", "Resolved", "Closed", "Mitigated"].index(incident.get_status()),
            key=f"stat_{incident.get_id()}"
        )

        if st.button("Update", key=f"update_{incident.get_id()}"):
            db.update_security_incident(
                incident.get_id(),
                new_severity,
                new_status
            )
        ...
        if st.button("🗑 Delete", key=f"delete_{incident.get_id()}"):
            db.delete_security_incident(incident.get_id())
